...
event.on("openApplication", res => {
    ...
    switch (res) {
        ...
        case "Jarvis":
            ...
            childProc.exec("open -a 'Google Chrome' --new --args https://localhost:" + user.port + " --ignore-certificate-errors");  
        ...
        case "Raspberry":
            //ssh pssh pi@192.168.1.103 "~/n/bin/node Desktop/hallwayPi/piServer.js"
            childProc.exec("ssh pi@192.168.1.103 'cd Desktop/hallwayPi;~/n/bin/node piServer.js'");
...
event.on("updateSpotifyStates", () => {
    ...
    spotify.getState((err, state) => {
        ...
        event.emit("trackTimeInfo", state);

    }); 

    spotify.getTrack((err, track) => {

        event.emit("spotifyTrackInfo", track);      
